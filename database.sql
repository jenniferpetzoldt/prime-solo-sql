...
-- 8. Add a new record.
        INSERT INTO accounts (username, city, transactions_completed, transactions_attempted, account_balance) VALUES ('jennifer', 'carthage', 20, 20, 65000.00);
-- 9. The bank is losing money in Miami and Phoenix and needs to unload low transaction customers: Delete users that reside in miami OR phoenix and have completed fewer than 5 transactions.
        SELECT * FROM "accounts" WHERE "city" LIKE 'phoenix' AND "transactions_completed" < '5';
        DELETE FROM "accounts" WHERE "user_id" = '10';
        SELECT * FROM "accounts" WHERE "city" LIKE 'miami' AND "transactions_completed" < '5';
        DELETE FROM "accounts" WHERE "user_id" = '6';